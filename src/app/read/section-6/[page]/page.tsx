...
import { section6 } from '@/data/bookData'
...
export default function Section6Page() {
    ...
    const currentPage = section6[pageNum - 1]
    const totalPages = section6.length
    ...
    useEffect(() => {
        ...
        // Save progress (Intro: 2 + S1: 11 + S2: 6 + S3: 9 + S4: 11 + S5: 10 = 49 pages total before Section 6)
        if (authed) {
            authSystem.updateReadingProgress(49 + pageNum).catch(err => {
    ...
    const handleNext = async () => {
        ...
        if (!isLastPage) {
            router.push(`/read/section-6/${pageNum + 1}`)
        } else {
            // Mark Section 6 as completed (Index 6)
            if (isAuthed) {
                await authSystem.completeChapter(6)
            }
            router.push('/library/1') // Go to next chapter
        }
    }
    ...
    const handlePrev = () => {
        if (!isFirstPage) {
            ...
            // Navigate back to last page of Section 5 (10 pages)
            router.push('/read/section-5/10')
        }
    }
    ...
    return (
        <>
            ...
            <LockedOverlay
                ...
                nextPath={`/read/section-6/${pageNum}`}
            ...
            <main style={{ minHeight: '100vh', position: 'relative', overflow: 'hidden', paddingTop: '0' }}>
                ...
                <div className="container" style={{ paddingBottom: '40px', maxWidth: '1400px' }}>
                    ...
                    <div className="reading-actions-bar">
                        <BookmarkButton 
                            pageId={`section-6-${pageNum}`} 
                            pageTitle={`الفصل 06: ${currentPage.title}`} 
                        />
                    </div>
